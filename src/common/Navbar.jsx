...
const Navbar = ({ type = "user", onSwitch }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [showNotifications, setShowNotifications] = useState(false);
  const [showProfile, setShowProfile] = useState(false);

  // Determine if we're currently in barber dashboard
  const isBarberDashboard = type === "barber" || location.pathname.startsWith("/barber");
  ...
  const handleAddBarber = () => {
    navigate("/barber/partners/create");
  };
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-lg border-b border-gray-100">
      <div className="flex justify-between items-center p-4 px-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-800">BharatNaai Admin</h1>
            <p className="text-xs text-gray-500">{isBarberDashboard ? "Barber Dashboard" : "User Dashboard"}</p>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-2xl mx-8">
          <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-12 pr-4 py-2.5 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-50 hover:bg-white transition-all duration-200"
            />
          </div>
        </div>
          ...
          >
            {isBarberDashboard ? (
              ...
              <>
                ...
                <span>Switch to Barber</span>
              </>
            )}
          </button>

          {/* Add Barber Button - Only visible in Barber Dashboard */}
          {isBarberDashboard && (
            <button
              onClick={handleAddBarber}
              className="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white px-4 py-2 rounded-lg font-semibold flex items-center gap-2 shadow-lg hover:shadow-xl transition-all duration-200"
            >
